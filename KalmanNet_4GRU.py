...
class KalmanNetNN_4GRU(torch.nn.Module):
    ...
    def NNBuild(self, SysModel, args):
        ...
        self.InitKGainNet(SysModel.prior_Q, SysModel.prior_Sigma, SysModel.prior_S,SysModel.prior_R, args)

    ######################################
    ### Initialize Kalman Gain Network ###
    ######################################
    def InitKGainNet(self, prior_Q, prior_Sigma, prior_S,prior_R, args):
        ...
        # GRU to track S
        self.d_input_S =  2* self.n ** 2
        self.d_hidden_S = self.n ** 2
        self.GRU_S = nn.GRU(self.d_input_S, self.d_hidden_S).to(self.device)

        # GRU to track R
        self.d_input_R = 2 * self.n * args.in_mult_KNet
        self.d_hidden_R = self.n ** 2
        ...
        # Fully connected 8
        self.d_input_FC8 = self.d_hidden_R
        self.d_output_FC8 = self.n ** 2
    ...
    def KGain_step(self, obs_diff, obs_innov_diff, fw_evol_diff, fw_update_diff):
        ...
        #R-GRU
        in_R = out_FC7
        out_R, self.h_R = self.GRU_R(in_R, self.h_R)

        # S-GRU

        in_S = torch.cat((out_R, out_FC1), 2)
        #print(out_FC1)
        #print( in_S.shape)
    ...
    def init_hidden_KNet(self):
        ...
        hidden = weight.new(self.seq_len_input, self.batch_size, self.d_hidden_R).zero_()
        self.h_R = hidden.data
        self.h_R = self.prior_R.flatten().reshape(1, 1, -1).repeat(self.seq_len_input, self.batch_size,
